...
def get_numeric_stats(df: DataFrame, _col: str, col_select: str, count: int) -> dict:
    stats = df.select(F.round(F.mean(col_select), 1).alias('mean'),
                      ...
                      (F.count(F.when(F.isnan(col_select) | F.col(col_select).isNull(), col_select)) / count)
                      .alias('null %')) \
        .collect()[0].asDict()
...
def get_datetime_stats(df: DataFrame, _col: str, col_select: str, _count: int) -> dict:
...
def get_stats_by_column(
        bucket_name: str, table_name: str, col_name: str,
        df: DataFrame, count: int) -> List[TableColumnStats]:
    column_stats = []
    start_epoch = str(datetime.now().timestamp())
    ...
    for stat in stats:
        column_stat = TableColumnStats(table_name=table_name,
                                       ...
                                       end_epoch=str(datetime.now().timestamp()),
                                       db='minio',
                                       cluster=bucket_name,
                                       schema='minio'
                                       )
        logging.info('get_stats_by_column: processing table: %s, column: %s',
                     table_name,
                     col_name)
        logging.debug('get_stats_by_column: column stats: %s',
                      column_stat.__dict__)
